...
void ObImage::LoadFile(wstring file)
{
    this->file = file;
    if (not sampler)
    {
    ...
    SetScale().x = imageSize.x;
    SetScale().y = imageSize.y;
}

void ObImage::Render(Camera* cam)
{
    if (!isVisible or !SRV)return;
